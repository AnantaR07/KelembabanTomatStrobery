...
import { getTomatHistory } from "./api/firebaseconfig";
...
function App() {
  const [tomatData, setTomatData] = useState([]);
  ...
  useEffect(() => {
    getTomatHistory((data) => {
      const latestData = data.slice(-6).reverse();
      setTomatData(latestData);
    });
  }, []); // Pemanggilan getTomatHistory() ditempatkan di useEffect terpisah

  const tomatChartData = {
    labels: tomatData.map((item) => item.waktu),
    datasets: [
      {
        label: "Kelembaban Tomat",
        data: tomatData.map((item) => item.kelembaban),
        borderColor: "red",
